...
def _run_id():
    segment = wiz.request.match("/api/runs/<run_id>/comments")
...
if request.method not in ("POST", "DELETE"):
    ...
    wiz.response.status(403, success=False, message="공개된 친구 기록에만 댓글을 남길 수 있습니다.")
else:
    payload = _payload()
    actor = _actor_profile(viewer_id)

    if request.method == "POST":
        item, message = running.save_run_comment(
            run_id,
            viewer_id,
        ...
        comment_id = payload.get("id") or payload.get("comment_id") or payload.get("commentId") or wiz.request.query("id", "")
        deleted, message = running.delete_run_comment(run_id, comment_id, viewer_id)
        if not deleted:
            wiz.response.status(400, success=False, message=message)
        else:
            profiles = _profiles_for_run(run, viewer_id, actor)
